...
async def test_api_mock_lead_chat_uses_process_harness_and_retains_session(tmp_path):
    """Exercise the user-facing Lead Chat path with the real mock process."""
    cfg = Settings(
        data_dir=str(tmp_path / "turn"),
        projects_dir=str(tmp_path / "projects"),
        planner="mock",
        default_executor="mock",
        runner_tick_seconds=0.02,
    )
    store = Store(tmp_path / "turn", projects_dir=tmp_path / "projects")
    await store.init()
    registry = WorkerRegistry()
    registry.register(MockHarnessWorker(cfg))
    registry.register_planner(MockHarnessPlanner(cfg), key="mock")
    runner = Runner(
        store,
        registry=registry,
        events=EventBus(),
    ...
    try:
        async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
            created = await client.post("/api/projects", json={
                "prompt": "Build a deterministic incident-response CLI",
                "agent": {"harness": "mock", "model": "deterministic"},
                "run_policy": {"auto_run": False},
            })
            assert created.status_code == 200, created.text
            project_id = created.json()["project_id"]
            sent = await client.post(
                f"/api/projects/{project_id}/lead/chat",
                json={"message": "What should I verify before shipping?"},
            )
            assert sent.status_code == 200, sent.text
            assert sent.json()["queued"] is True
            assert sent.json()["message"]["role"] == "user"
            ...
            for _ in range(100):
                ...
                if any(item["role"] == "lead" for item in transcript):
                    break
                await asyncio.sleep(0.05)
            assert transcript is not None
            replies = [item for item in transcript if item["role"] == "lead"]
            assert replies and "process-level harness" in replies[-1]["content"]
            user_message = next(
                item for item in transcript
                if item["content"] == "What should I verify before shipping?"
            )
            assert user_message["status"] == "CONSUMED"
            assert user_message["run_id"] is not None
            lead = (await client.get(f"/api/projects/{project_id}/lead")).json()["lead"]
            runs = await store.get_runs(uuid.UUID(lead["terminal_owner_id"]))
            assert len(runs) == 1
            assert runs[0].status is RunStatus.COMPLETE
            assert runs[0].session_id == lead["session_id"]
    finally:

...
class TurnRuntime:
    ...
    async def start(self) -> RuntimeComponents:
        ...
        try:
            ...
            if not self.test_mode:
                # Production organization acceptance is hierarchical: the
                # parent planner (nested) or project lead (root) runs one
                # bounded review turn through the scheduler/step frontier.
                # Test mode stays provider-neutral unless a test injects
                # callbacks into Runner directly.

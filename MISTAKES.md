...
## 2026-08-21 — lead inspector review (user caught both)

1. **The lead panel got a special-purpose "Send the lead a message" box.** That duplicates the terminal — agents are fully interactive sessions and humans steer them by typing into their pane, same as every other node. The messaging UI, `/lead/message` endpoint, `LeadMessage` model, and client API were removed; interaction is the terminal. Lesson: when a surface already exists for a concern (the interactive terminal), adding a second surface for the same concern is a design violation, not a convenience.
2. **`.detail` collapsed to 34px in the lead inspector, hiding everything below.** Root cause: `.inspector` is `grid-template-rows: 38px 34px minmax(0,1fr)` (heading / tabs / content); the lead panel has no tab strip, so its content fell into the 34px tabs row. The trigger inspector had an override; the lead one didn't. Fix: `.inspector.lead-inspector { grid-template-rows: 38px minmax(0,1fr) }`. Lesson: shared layout shells must be verified per-variant with rendered geometry, not assumed from class reuse.
